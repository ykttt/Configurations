...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
                /* layout symbol in front of tag symbols */
...
cleanup(void)
{
	...
	for (i = 0; i < LENGTH(col_schemes); i++)
		free(scheme[i]);
	free(scheme);
...
deck(Monitor *m) {
	...
	if(n > m->nmaster) {
		...
		if(i < m->nmaster) {
			...
			resize(c, m->wx + gappx, m->wy, m->ww - mw - (2*c->bw) - gappx*(5-ns)/2, m->wh - (2*c->bw), 0);
}
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		/* 	drw_rect(drw, */
		/*                             x + boxw + tag_label_loffset, */
		/*                             tag_label_vertoffset, */
		/*                             w - ( 2 * boxw + tag_label_roffset + 1), */
		/*                             boxw / tag_label_hsqueeze, */
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			/*                                     x + boxw + tag_label_loffset, */
			/*                                     tag_label_vertoffset, */
			/*                                     x + boxw + tag_label_loffset + tag_label_float, */
			/*                                     boxw / tag_label_hsqueeze, */
			/*             m->sel->isfixed, */
			/*             0); */
				/* drw_rect(drw, x + boxs, boxs, boxw, boxw / tag_label_hsqueeze, m->sel->isfixed, 0); */
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
		        tlpad = MAX((m->ww - ((int)TEXTW((char*)bar_default_title) - lrpad)) / 2 - x, lrpad / 2);
                        drw_text(drw, x, 0, w - 2 * sp, bh, tlpad, (char*)bar_default_title, 0);
...
setup(void)
{
	...
	drw = drw_create(dpy, screen, root, sw, sh, visual, depth, cmap);
        if (!drw_fontset_create(drw, bar_fonts, LENGTH(bar_fonts)))
                die("no fonts could be loaded.");
	lrpad = drw->fonts->h + bar_horipad;
        bh = drw->fonts->h + bar_vertpad;
        sp = bar_horigap;
        vp = (bar_position == 1) ? bar_vertgap : - bar_vertgap;
	updategeom();
	/* init atoms */
	...
	if (LENGTH(tags) > LENGTH(col_tag_sel_schemes))
		die("too few color schemes for the tags");
	scheme = ecalloc(LENGTH(col_schemes), sizeof(Clr *));
	for (i = 0; i < LENGTH(col_schemes); i++)
                scheme[i] = drw_scm_create(drw, col_schemes[i], alph_schemes[i], 3);
	tagscheme = ecalloc(LENGTH(col_tag_sel_schemes), sizeof(Clr *));
	for (i = 0; i < LENGTH(col_tag_sel_schemes); i++)
		tagscheme[i] = drw_scm_create(drw, col_tag_sel_schemes[i], alph_tag_sel_schemes, 2);
	/* init bars */
	updatebars();
	updatestatus();
...
updatebarpos(Monitor *m)
{
	...
	m->wh = m->mh;
	if (m->showbar) {
                m->wh = m->wh - bar_vertgap - bh;
                m->by = m->topbar ? m->wy : m->wy + m->wh + bar_vertgap;
