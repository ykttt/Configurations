...
setup(void)
{
	...
	lrpad = drw->fonts->h + bar_horipad;
        ...
        sp = bar_horigap;
        vp = (topbar == 1) ? bar_vertgap : - bar_vertgap;
	updategeom();
	/* init atoms */
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + m->ww - mw + gappx, m->wy + my, mw - (2*c->bw) - gappx*(5-ns)/2, h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c) + gappx;
		} else {
			h = (m->wh - ty) / (n - i) - gappx;
			resize(c, m->wx + gappx/ns, m->wy + ty, m->ww - mw - (2*c->bw) -gappx*(5-ns)/2, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c) + gappx;
		}
}
